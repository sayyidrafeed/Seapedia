...
import { privateAdminEndpoint } from '@/lib/api/generated';
...
function AdminDashboard() {
  ...
  const testPrivateEndpoint = async () => {
    ...
    setError(null);
    try {
      const { data, error: apiError } = await privateAdminEndpoint();
      if (apiError) {
    ...
    <div className="container mx-auto px-6 py-12 space-y-6">
      ...
      <div className="bg-card border border-border p-6 rounded-lg shadow-sm">
        ...
        <p className="text-sm text-muted-foreground mb-4">
          Test the private endpoint that only allows users with the Admin role.
        </p>
        <button
        ...
        >
          {loading ? 'Loading...' : 'Test Private Admin Endpoint'}
        </button>
        {result && (
